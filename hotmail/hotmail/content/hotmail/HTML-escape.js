function HTMLescape(oLog)
{
    ...
    this.kBlockSize = 5500;
    this.m_iTimer = 30;
    this.kTerminator = "&#13;&#10;";
    this.kTerminatorAlt = "\r\n";
    this.kTerminatorAlt2 = "&#10;";
...
HTMLescape.prototype.notify = function (timer)
{
    try
    {
        ...
        if (iComplete ==-1)
        {
        	this.m_Log.Write("HTMLescape.js - notify - Failed kTerminator");  
            iComplete = this.m_szTempMSG.lastIndexOf(this.kTerminatorAlt)
            if (iComplete==-1)
        	{
            	this.m_Log.Write("HTMLescape.js - notify - Failed kTerminatorAlt");  
            	iComplete = this.m_szTempMSG.lastIndexOf(this.kTerminatorAlt2)
                if (iComplete==-1)
                {
                	this.m_Log.Write("HTMLescape.js - notify - Failed kTerminatorAlt2"); 
            		iComplete = this.kBlockSize;
                }
                else
                	iComplete += this.kTerminatorAlt2.length;
        	}
            else
