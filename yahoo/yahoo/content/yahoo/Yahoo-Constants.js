...
const patternYahooNewLogin = /<fieldset id="mbr-login-fieldset".*?>[\S\s]*<\/fieldset?/gm;
const patternYahooAction = /<form.*?action="(.*?)"[\S\s]*?>/;
const patternYahooInput = /<input.*?name=.*?type=['|"]*hidden['|"]*.*?>/igm;
...
const kPatternFolderData =  /"folderInfo":\{"fid":".*?","name":".*?",/img;
const kPatternFolderList=/\{"numberOfFolders"\:\d+,"folder":\[[\s\S]*?\]\};/igm;
const kPatternFolderName =/"name"\:"(.*?)"/i;
const kPatternFolderID =/"fid"\:"(.*?)"/i;
const kLstMsgs = "{\"method\":\"ListFolderThreads\",\"params\":\[{\"fid\":\"MSGLIST\",\"sortKey\":\"date\",\"sortOrder\":\"down\",\"start\":0,\"numCInfos\":100}\]}";
const kPatternInfo = /"conversations"\:[\s\S]*?"error"\:null}/ig;
const kPatternFolderInfo =/({"conversation"\:[\s\S]*?}}\]}},)/i;
const kPatternFolderSource =/"sourceFolderInfo"\:{"fid":".*?","name"\:"(.*?)"/i;
...
const kSendMessge ="{\"method\":\"SendMessage\",\"params\":[{\"message\":{\"subject\":\"EMAILSUBJECT\",\"from\":{\"email\":\"FROMADDRESS\",\"name\":\"FROMNAME\"},\"to\":[TOADDRESS],\"cc\":[CCEMAILADDRESS],\"bcc\":[BCCEMAILADDRESS],\"replyto\":{\"name\":\"FROMNAME\",\"email\":\"FROMADDRESS\"},\"simplebody\":{\"html\":HTMLMSG,\"text\":PLAINMSG,\"attachment\":[EMAILATTCHMENTS]}}}]}";
const kPatternSendMSGResponse = /"error":null/i;
...
const kPatternCred =/"cred"\:"(.*?)"/i;
const kFileId = /\<id\>(.*?)\<\/id\>/i;
